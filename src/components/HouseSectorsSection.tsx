...
export default function HouseSectorsSection({
  ...
  return (
    <section id="our-house" className="py-24 bg-[#071526] relative overflow-hidden">
      ...
      >
        {/* Section Header */}
        <motion.div variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0, transition: { duration: 0.6 } } }} className="text-center max-w-3xl mx-auto space-y-4 mb-16">
        ...
        {/* Divisions 6-Card Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {DIVISIONS.map((division) => (
            <motion.div
              variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}
              key={division.id}
              className="group relative bg-[#0B1C31]/90 hover:bg-[#0E243D] border border-slate-800 hover:border-[#c5a869]/60 rounded-xl p-6 sm:p-7 transition-all duration-300 flex flex-col justify-between hover:shadow-[0_10px_30px_rgba(0,0,0,0.5)] hover:-translate-y-1"
            >
